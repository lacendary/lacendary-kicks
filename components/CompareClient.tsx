...
export default function CompareClient({
  ...
  const [leftShoe, setLeftShoe] = useState(
    lockedSneaker ?? sneakers[0]
  );

  const [rightShoe, setRightShoe] = useState(
    sneakers.find(
      (shoe) => shoe.slug !== (lockedSneaker?.slug ?? sneakers[0].slug)
    ) ?? sneakers[1]
  );
...
<div className="grid grid-cols-1 items-center gap-6 lg:grid-cols-[1fr_auto_1fr]">
  ...
  <SneakerSelector
    ...
    label={lockedSneaker ? "Current Sneaker" : "Select Sneaker"}
  />
